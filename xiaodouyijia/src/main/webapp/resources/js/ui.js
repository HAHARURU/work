...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    if(!this.fileProgressWrapper.find('td:eq(2) .btn-default').length){
        this.fileProgressWrapper.find('td>div').append("");
    }
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    //var showImg = $('<img src="/resources/image/loading.gif"/>');
    var showImg = $('');

    //progressNameTd.append(Wrapper);

    if (!isImg) {
        //showImg.attr('src', '/resources/image/default.png');
        ...
        $(img).on('load', function() {
            ...
            function initImg(url, key, height) {
                ...
                //modalBody.find('img').attr('src', '/resources/image/loading.gif');
